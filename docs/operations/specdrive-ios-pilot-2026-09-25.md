...
第三轮 [run 36157364558](https://github.com/horton2048/milo/actions/runs/36157364558)，源提交 `d65c577f31bdeb646a47e6bc02794375055181e0`，首次真正通过全部六项 UI 测试并输出六张截图。独立审图确认大字号、长中文及键盘/保存可达，但发现三个标为 Dark 的截图实际仍为浅色：首页和详情各自的 Light/Dark 文件完全相同。未发布任何通过回执；保留证据后取消等待中的任务。下一轮将用仅限 DEBUG 且有效 UUID 夹具的 SwiftUI 外观覆盖、实际环境模式断言和重新审图验证深色，而不依赖无效的启动参数。
...
已证实的收益是失败可追踪、旧证据不能冒充当前结果、缺少真实环境时不会误报完成。代价是要维护规格、检查配置、项目专用脚本、工具版本和 CI 环境，并承担原生测试和审图时间。本轮没有与其他框架的对照实验，不能据此声称开发更快、更省费用或是普遍最佳方案。
